...
; Every time it toggles to "INSERT COIN," the computer plays a piece. There is
; no sound. After 7 moves are played, the program resets.
    CALL CLEAR_MESSAGE
    CALL DRAW_MESSAGE
    DB $01                         ; unused arg
...
; Flash the 'INSERT ￥100' message and check the coin slot for roughly one
...
DRAW_CHARACTER:
; After the character is drawn, the VRAM_POINTER is advanced 6 pixels to the
; right.
    PUSH H                         ; save VRAM_POINTER
    CALL DRAW_5x7_IMAGE
    POP H                          ; restore VRAM_POINTER
...
MESSAGE_FONT_TABLE:
; Some message strings use dakuten (゛) and handakuten (゜) marks. These are
; stored as separate glyphs which modify the preceding kana character,
; e.g. カ + ゛ = ガ and ハ + ゜ = パ.
; 00 = イ
    DB $08                         ; --------xx
    DB $10                         ; ------xx--
